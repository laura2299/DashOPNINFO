...
def Grafica_barras_eventos(tabla,columnas,filtro_datos,cadena):
    nro_columnas=len(columnas)-1
    ...
    def convertir_horas(h1,h2):
        ...
        hora1 = datetime.strptime(h1, '%H:%M')
        hora2 = datetime.strptime(h2, '%H:%M')
...
def elije_embalses(container,columnas,tipos):
    for widget in container.winfo_children():
        widget.destroy()
    global filtros_form_grafico
    ...
    for txt in listdir("Cache\Cache_Graficos"):
        with open("Cache/Cache_Graficos/"+txt,"r",encoding="utf-8") as archivo:
            for linea in archivo:
                ...
                for i in range(len(nombres)):
                    if linea[posicion_cad]==nombres[i]:
                        diferencia=convertir_horas(linea[posicion_I],linea[posicion_F])
                        subsuma[i]=subsuma[i]+float(diferencia)
        ...
        if type(filtro_form)== type(columnas):
            ...
            if len(a.get())==4:
                if(columnas[3]=="DÍA"):
                    ...
                    fechainicial=str(a.get()+"/"+m[0].get()+"/"+d[0].get())
                    fechafinal=str(b.get()+"/"+m[1].get()+"/"+d[1].get())
                else:
                    m=filtros_form_grafico[1]
    ...
    for txt in listdir("Cache\Cache_Graficos"):
        with open("Cache/Cache_Graficos/"+txt,"r",encoding="utf-8") as archivo:
            for linea in archivo:
                ...
                if columnas_a[1]=="AÑO" and columnas_a[1]=="MES" and columnas_a[2]=="DÍA":
        ...
        with open("Cache/Cache_Graficos/"+txt,"r",encoding="utf-8") as archivo:
            for linea in archivo:
                ...
                for i in range(len(nombres)):
                    ...
                    if linea[posicion_cadena]==nombres[i] and columna_posicion_float !=0 and nombre_tabla=="EVENTOS":
                        print("hacer funcion de la diferencia de horas y añadir a subsuma")
...
def elige_torta(container,columnas,filtro_datos,res,columnas_grafico):
    ...
    tk.Button(
            ...
            command=lambda:graficar_datos_torta_horas(res,columnas_grafico),
            **style.STYLE2,
            relief= tk.FLAT,
            activebackground=style.BACKGROUND,
            activeforeground= style.TEXT,
...
def graficar_datos_torta_horas(res,columnas_grafico):            
    print(res)
        ...
        with open("Cache/Cache_Graficos/"+txt,"r",encoding="utf-8") as archivo:
            for linea in archivo:
                ...
                for i in range(len(nombres)):
                    if linea[posicion_cadena]==nombres[i]:
                        for j in range(24):
                            sub_suma=sub_suma+float(linea[posicion_H+j])
                            #print("j:"+str(j))
            ...
            for linea in archivo:
                ...
                for i, nombre in enumerate(nombres):
                    ...
                    if linea[posicion_cadena]==nombres[i]:
                        ...
                        for j in range(24):
                            horas[i][j]=horas[i][j]+float(linea[posicion_H+j])
